...
// ─── Server setup ──────────────────────────────────────────────────────────────
const server = new Server({ name: "cold-outreach-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
    ...
    try {
        switch (name) {
            ...
            case "find_company_email":
                result = await handleFindEmail(args);
                break;
            case "research_company":
                result = await handleResearchCompany(args);
                break;
            case "generate_email":
                result = await handleGenerateEmail(args);
                break;
            case "send_email":
            ...
            case "bulk_outreach":
                ...
                break;
            case "log_to_notion":
                result = await handleLogToNotion(args);
                break;
            case "retry_failed":
                result = await handleRetryFailed(args);
                break;
            default:
                return {
