...
export async function setupDatabase(): Promise<string> {
  ...
  if (!dbId) {
    throw new Error(
      "NOTION_DATABASE_ID is not set. Create a database on Notion, copy its ID from the URL, and add it to your .env and Claude Desktop config."
    );
  }
  ...
  if (!checkRes.ok) {
    ...
    throw new Error(
      `Cannot access Notion database (${checkRes.status}): ${err}.\n` +
