...
export const retryFailedSchema = {
  name: "retry_failed",
  description:
    "Finds all outreach records marked as 'failed' in Notion and re-sends the emails. Updates status to 'sent' or keeps 'failed' with updated notes.",
  inputSchema: {
    type: "object",
    properties: {
      subject: {
...
export async function handleRetryFailed(args: {
  ...
  const results: Array<{ company: string; email: string; result: string }> = [];
